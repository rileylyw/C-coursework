...
void read(char file[], int *ySize, int *xSize){
   FILE *fp=fopen(file, "r");
   if(!fp){
      perror("Couldn't open file\n");
   }
   if(fscanf(fp, "%d%d", ySize, xSize)<0){ //can't read first 2 num
      return;
   }
   for(int i=0; i<*ySize; i++){
      if(fscanf(fp, "%s", data[i])<0){ //can't read lines
         return;
      }
   }
}
...
void verboseCondition(int argc, char *argv[], char file[]){
   if(argc==3){
      strcpy(file, argv[2]);
   }
   else if(argc==2){
...
int main(int argc, char *argv[]){
   int ySize, xSize;
   char file[52];
   verboseCondition(argc, argv, file);
   read(file, &ySize, &xSize);
   bfs();
}
